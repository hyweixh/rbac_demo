...
def generate_jwt(user):
    timestamp = int(time.time()) + 60*60*8   #设置8小时过期
...
class UserTokenAuthentication(BaseAuthentication):
    """
    在实际视图处理过程中，直接使用中间件已经设置好的用户信息，简化了认证逻辑。
    """
    def authenticate(self, request):
        """
        request._request.user：表示已经在请求中解析出的用户对象。
        request._request.auth：表示已经在请求中解析出的认证信息（例如，Token）。
        """
        # print("用户名:",request._request.user)
        # print("Token:",request._request.auth)
...
#             try:
#                 user = opsUser.objects.get(pk=userid)
